...
parser.add_argument('--label_ex_path',
                    type=str,
                    default='eval_tools/DarkFaceResult', help='label export path')
parser.add_argument('--test_img_path',
                    type=str,
                    default='/tmp2/biolin/cvprw_llfd/DarkFace_Train/images_test',
                    help='test img dir')
...
if __name__ == '__main__':
    # fnames = glob('/home/r07944011/datasets/cvprw_llfd/DarkFace_Train/images_test/*')
    fnames = glob(args.test_img_path + '/*')
    save_path = args.label_ex_path
    ...
    counter = 0
    # fnames = ['/home/r07944011/datasets/cvprw_llfd/DarkFace_Train/images_test/5097.png']
    for ii, in_file in enumerate(tqdm(fnames)):
        im_name = os.path.basename(in_file)
        im_name = os.path.splitext(im_name)[0] + '.txt'
        ...
        # except Exception as e:
        #    print('>>> predicting image: %s' % im_name)
        #    print(img)
        #    print(img.shape)
        #    print(max_im_shrink)
        #    print(e)
        det = np.row_stack((det0, det1, det2, det3))
        # print(str(ii), det.shape)
        try:
            dets = bbox_vote(det)
        except Exception as e:
            print('>>> Exception in bbox_vote, save empty file:', e)
            fout = open(osp.join(save_path, im_name), 'w')
            fout.write('\n')
            continue
        t2 = time.time()
        ...
        for i in range(dets.shape[0]):
            ...
            ymax = dets[i][3]
            score = dets[i][4]
            fout.write('face {:.3f} {:.1f} {:.1f} {:.1f} {:.1f}\n'.
